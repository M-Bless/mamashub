...
class FamilyPlannigEdit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("FamilyPlannigEdit", "Submit request received")
    ...
    private suspend fun fetchAndPopulateQuestionnaireResponse(responseId: String) {
        retrofitCallsFhir.fetchQuestionnaireResponse(responseId, object : Callback<ResponseBody> {
                    ...
                    if (questionnaireResponseString != null) {
                        CoroutineScope(Dispatchers.Main).launch {
                            try {
                                ...
                                Log.e("FamilyPlannigEdit", "Error populating questionnaire", e)
                                Toast.makeText(this@FamilyPlannigEdit, "Error populating questionnaire", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@FamilyPlannigEdit, "Failed to retrieve the response data.", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@FamilyPlannigEdit, "Failed to fetch the questionnaire response: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("FamilyPlannigEdit", "Failed to fetch response. Response code: ${response.code()}")
                    }
                }
            }
            ...
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                CoroutineScope(Dispatchers.Main).launch {
                    Toast.makeText(this@FamilyPlannigEdit, "Error occurred while fetching: ${t.message}", Toast.LENGTH_SHORT).show()
    ...
    private fun populateQuestionnaireFragment(questionnaireResponse: QuestionnaireResponse) {
        try {
            ...
            Log.d("FamilyPlannigEdit", "Questionnaire response populated successfully.")
        } catch (e: Exception) {
            Log.e("FamilyPlannigEdit", "Error initializing the questionnaire fragment or ViewModel", e)
        ...
        if (updatedQuestionnaireResponse != null) {
            ...
            retrofitCallsFhir.updateQuestionnaireResponse(responseId, updatedResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@FamilyPlannigEdit, "Data updated successfully.", Toast.LENGTH_SHORT).show()
                            finish()
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@FamilyPlannigEdit, "Failed to update data: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@FamilyPlannigEdit, "Error occurred while updating: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            })
        } else {
