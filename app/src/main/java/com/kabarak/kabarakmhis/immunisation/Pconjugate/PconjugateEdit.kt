...
class PconjugateEdit: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_child_pnc_edit)
        ...
        questionnaireJsonString = getStringFromAssets("Conjugate-nb-NO-v1.3.json")
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("PconjugateEdit", "Submit request received")
    ...
    private suspend fun fetchAndPopulateQuestionnaireResponse(responseId: String) {
        ...
        retrofitCallsFhir.fetchQuestionnaireResponse(responseId, object : Callback<ResponseBody> {
                    ...
                    if (questionnaireResponseString != null) {
                        CoroutineScope(Dispatchers.Main).launch {
                            try {
                                ...
                                Log.e("PconjugateEdit", "Error populating questionnaire", e)
                                Toast.makeText(this@PconjugateEdit, "Error populating questionnaire", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@PconjugateEdit, "Failed to retrieve the response data.", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@PconjugateEdit, "Failed to fetch the questionnaire response: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("PconjugateEdit", "Failed to fetch response. Response code: ${response.code()}")
                    }
                }
            }
            ...
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                CoroutineScope(Dispatchers.Main).launch {
                    Toast.makeText(this@PconjugateEdit, "Error occurred while fetching: ${t.message}", Toast.LENGTH_SHORT).show()
    ...
    private fun populateQuestionnaireFragment(questionnaireResponse: QuestionnaireResponse) {
        try {
            ...
            Log.d("PconjugateEdit", "Questionnaire response populated successfully.")
        } catch (e: Exception) {
            Log.e("PconjugateEdit", "Error initializing the questionnaire fragment or ViewModel", e)
        ...
        if (updatedQuestionnaireResponse != null) {
            ...
            retrofitCallsFhir.updateQuestionnaireResponse(responseId, updatedResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@PconjugateEdit, "Data updated successfully.", Toast.LENGTH_SHORT).show()
                            // End the activity after successful submission
                            finish()
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@PconjugateEdit, "Failed to update data: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@PconjugateEdit, "Error occurred while updating: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            })
        } else {
