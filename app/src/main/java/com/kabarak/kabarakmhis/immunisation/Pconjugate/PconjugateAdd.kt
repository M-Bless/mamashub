...
class PconjugateAdd : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_pconjugate_add)
            ...
            retrofitCallsFhir.submitQuestionnaireResponse(questionnaireResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@PconjugateAdd, "Successfully submitted!", Toast.LENGTH_SHORT).show()
                        Log.d("ChildAdd", "Successfully submitted the questionnaire response.")
                    } else {
                        Toast.makeText(this@PconjugateAdd, "Submission failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("Error", "Failed to submit. Response code: ${response.code()}")
                    }
                }

                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    Toast.makeText(this@PconjugateAdd, "Error occurred while submitting: ${t.message}", Toast.LENGTH_SHORT).show()
